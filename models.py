...
class OracleModel():
  ...
  def create_model(self, observations: list):
    board_size = self.board.rows * self.board.cols
    num_turns = len(observations)
    with pm.Model() as model:
      # this is our prior guess on whether or not they are an alien.
      is_alien = pm.Bernoulli(
          'is_alien',
          p=0.5,
        )
